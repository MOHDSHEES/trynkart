...
const wishlistItems = (userId) => async (dispatch) => {
  try {
    dispatch({ type: "ITEM_WISHLIST_REQUEST" });
    const { data } = await axios.post("/api/user/wishlistitems/" + userId);
    dispatch({ type: "ITEM_WISHLIST_SUCCESS", payload: data });
  } catch (error) {
    dispatch({ type: "ITEM_WISHLIST_FAIL", payload: error.message });
  }
};
// adding item to wishlist
const addToWishlist =
...
const signin = (Username, Password) => async (dispatch) => {
  try {
    ...
    dispatch({ type: "CART_ITEMS_SUCCESS", payload: cart });
    dispatch({ type: "ITEM_ADD_TO_WISHLIST_SUCCESS", payload: wishlist });
...
const signup = (username, email, password, mobileno) => async (dispatch) => {
  try {
    ...
    dispatch({
      type: "ITEM_ADD_TO_WISHLIST_SUCCESS",
...
const logout = () => async (dispatch) => {
  ...
  dispatch({ type: "SIGNIN_DETAILS_SUCCESS", payload: null });
  dispatch({ type: "CART_ITEMS_SUCCESS", payload: [] });
  dispatch({ type: "ITEM_ADD_TO_WISHLIST_SUCCESS", payload: [] });
