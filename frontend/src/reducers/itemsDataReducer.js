...
function itemsListReducer(state = { items: [] }, action) {
  switch (action.type) {
    case "ITEM_LIST_REQUEST":
      return { loading: true, items: state.items.length ? state.items : [] };
...
function itemAddToWishlistReducer(state = { wishlist: [] }, action) {
  switch (action.type) {
    ...
    case "ITEM_ADD_TO_WISHLIST_FAIL":
      return { load: false, err: action.payload };
    case "ITEM_WISHLIST_REQUEST":
      return { load: true, wishlist: [] };
    case "ITEM_WISHLIST_SUCCESS":
      return { load: false, wishlist: action.payload };
    case "ITEM_WISHLIST_FAIL":
      return { load: false, err: action.payload };
    default:
      return state;
  }
